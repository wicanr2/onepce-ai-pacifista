...
### 授權路線：B（使用者裁定 2026-09-04）

條款維持 **RRSAL-1.0**。Mesen2 是 GPL-3.0，與 RRSAL 不相容，所以對它的使用有三條
**[HARD]** 紀律，違反任何一條整個 repo 就得改掛 GPL：

1. **只取行為事實，不翻碼。** 從 Mesen2 讀出的東西限於：暫存器語意、週期表、旗標規則、
   時序順序、邊界條件。這些寫成本專案 `docs/spec/*.md` 裡的規則表（附 Mesen2 檔名與
   commit），Go 程式照 spec 寫，**不對照 Mesen2 的函式逐行寫**。
2. **不複製表達。** 不複製 Mesen2 的函式切分、變數名、註解、switch 結構或 lookup table
   的排列；opcode 表要用本專案自己的資料格式（例如 JSON／Go 結構）重新錄入，
   來源欄寫「公開指令集文件」，Mesen2 只作交叉核對。
3. **每個模組檔頭記錄參考**：`參考行為：Mesen2 <檔> @ b9fa69d §<主題>`，讓爭議時能證明
   拿了什麼、沒拿什麼。

結構參考改用 **ares**（ISC，允許併入專有作品，只要求保留版權聲明）：本機副本
`~/cht/tmp/ares`（commit `7b51c8ab719e403a150aa700e0933d9e93a06851`，2026-08-30），
PCE 核心在 `ares/pce/`（`cpu/`、`vdp/`（含 `vce.cpp`、`vdc.cpp`、`dma.cpp`、`sprite.cpp`）、
`psg/`、`cartridge/`、`system/`）。ares 的檔案切分與狀態機可以照著寫，併入的部分在
`NOTICE.md` 列 ares 的版權聲明。Mesen2 與 Mednafen 仍是**執行期 oracle**。

實務上的判準：一段 Go 碼如果拿掉 Mesen2 就寫不出來，那它就是翻的；如果拿掉 Mesen2
只是少了一個核對點，那就是行為事實。拿不準的寫進 spec 的未知表，先問。
...
M0–M6 於 2026-09-05 完成（M6 為第一版；現況見 `CONTEXT.md`）。
